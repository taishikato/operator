...
Caveat (TEXT primary key is nullable): Drizzle Kit beta (`1.0.0-beta.22`) does **not** emit `NOT NULL` on a `TEXT PRIMARY KEY`, regardless of `.notNull()` in the schema. Because SQLite/Turso allow NULL in a non-integer primary key, the exported DDL that Atlas applies accepts a NULL `id` (verified: `INSERT ... (id, ...) VALUES (NULL, ...)` succeeds against the exported schema). The spike originally created its table with hand-written SQL that added `NOT NULL` to `id`, which masked this and gave false confidence in the workflow. The spike SQL now matches the exported DDL exactly, so the test reflects what the real Drizzle → export → Atlas pipeline produces. Consequence for the MVP: the non-null `id` invariant is enforced at the application/TypeScript layer (ids are always supplied and `.notNull()` makes the Drizzle types treat `id` as non-null), not by a DB-level constraint. If a DB-level guarantee is wanted, the `operator db apply` path must add it explicitly (e.g. a `CHECK` or an Atlas-side schema source), since Drizzle Kit cannot express it for a TEXT PK.
