...
description: Use when asked to file, list, inspect, archive, or send tasks on the user's Cursor Operator board, or when work surfaces a follow-up worth queueing for Cursor Cloud Agent.
---

# Cursor Operator board CLI

Cursor Operator is a local macOS Kanban board that prepares and sends tasks
to Cursor Cloud Agent. Drive it with the `cursor-operator` CLI only. Do not read or write its SQLite database directly; the CLI uses the same
CursorOperatorCore domain code as the app.

## Setup check

Run `cursor-operator --help`. If the binary is missing, install it from the
repo:
...
| Archive a task | `cursor-operator task archive <task-id> --json` |
| Send a Ready task to Cursor | `cursor-operator task send <task-id> [--wait] --json` |
| List a task's run attempts | `cursor-operator run list --task <task-id> --json` |

The prompt is sent to Cursor Cloud Agent verbatim, so write it as a complete,
standalone instruction. Resolve repository names via `repo list` first;
ambiguous names require the id.

If the repository is not on the board yet, register it yourself with
`repo add <path>`. The path can be anywhere inside the Git working tree. The
CLI resolves the repository root, requires a GitHub origin remote, and infers
the default branch.

## Storage

Cursor Operator stores data under Application Support `Cursor Operator`.
For tests or sandboxed agent work, set `CURSOR_OPERATOR_DB=/path/to/db.sqlite`.
This is Cursor-specific and independent from codex_operator's storage.

## Lifecycle rules

- Statuses: `ready` -> `running` -> `done` or `failed`; `archived` removes a
  task from the active board.
...
- Archive is the only manual removal. There is no hard delete.
...
With `--json`, errors print
`{"error":{"code":"...","message":"..."}}` on stdout and still set the exit
code.

## JSON shapes

- repository: `id`, `name`, `localPath`, `githubURL`, `defaultBranch`,
  `createdAt`, `updatedAt`
...
- Confusing this with `operator`; `operator` sends to Codex, while
  `cursor-operator` sends to Cursor Cloud Agent.
- Retrying a `lifecycleViolation`; failed sends are retryable, lifecycle
  violations are not.
